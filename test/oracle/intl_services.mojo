...
from tsonic_js import (
    IntlCollator, IntlDateTimeFormat, IntlFormatPart, IntlNumberFormat,
...
def append_optional(mut names: List[JsString], mut values: List[JsValue], name: String, value: Optional[Float64]):
    if value:
        names.append(JsString(name))
        values.append(JsValue(value.value()))


def append_optional(mut names: List[JsString], mut values: List[JsValue], name: String, value: Optional[String]):
    if value:
        names.append(JsString(name))
        values.append(JsValue(JsString(value.value())))


def number_options_value(options: IntlResolvedNumberFormatOptions) raises -> JsValue:
    ...
    append_optional(names, values, "currencyDisplay", options.get_currency_display())
    ...
    append_optional(names, values, "minimumFractionDigits", options.get_minimum_fraction_digits())
    append_optional(names, values, "maximumFractionDigits", options.get_maximum_fraction_digits())
    append_optional(names, values, "minimumSignificantDigits", options.get_minimum_significant_digits())
    append_optional(names, values, "maximumSignificantDigits", options.get_maximum_significant_digits())
    var grouping = options.get_use_grouping()
    names.append(JsString("useGrouping"))
    values.append(JsValue(grouping[Bool]) if grouping.isa[Bool]() else JsValue(JsString(grouping[String])))
    names.append(JsString("notation"))
    values.append(JsValue(JsString(options.get_notation())))
    append_optional(names, values, "compactDisplay", options.get_compact_display())
...
def evaluate_service(record: JsValue, operation: String) raises -> JsValue:
    ...
    if operation == "numberFormat" or operation == "numberParts" or operation == "numberResolved":
    ...
    if operation == "dateFormat" or operation == "dateParts" or operation == "dateResolvedBase":
        ...
        if operation == "dateResolvedBase":
            ...
            return record_value("locale|calendar|numberingSystem|timeZone", values^)
    ...
    if operation == "collatorCompare" or operation == "collatorResolved":
        ...
        if operation == "collatorCompare":
            var order = collator.compare_units(field(record, "value").string_value(), field(record, "right").string_value())
        ...
        return record_value("locale|usage|sensitivity|ignorePunctuation|collation|numeric|caseFirst", values^)
