...
def escaped() raises -> TypedLocation[Int32]:
    var owner = JsArray[Int32](List[Int32](4, 5))
    return array_location(owner, 1.0)


def main() raises:
    var owner = JsArray[Int32](List[Int32](1, 2))
    ...
    for index in range(2048):
        _ = owner.push(List[Int32](Int32(index)))
    first.write(7)
    assert_equal(owner[0.0], 7)
    assert_equal(same.read(), 7)
    owner = JsArray[Int32](List[Int32](20))
    first.write(8)
    ...
    for invalid in List[Float64](-1.0, 0.5, 4294967295.0, Float64("nan")):
        var rejected = False
        try:
