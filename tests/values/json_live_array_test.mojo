...
from tsonic_js import JsString, JsValue, js_value_from_source_array, js_value_from_source_object, json_stringify_with_space_number
...
struct ArrayView:
    ...
    def has(context: ErasedCallableContext, var arguments: Tuple[Int]) -> Bool:
        return arguments[0] >= 0 and arguments[0] < len(context.unsafe_bitcast[Self]()[].owner[].values)
...
struct ItemView:
    ...
    @staticmethod
    def object_length(_context: ErasedCallableContext, var _arguments: Tuple[]) -> Int:
        return 0

    @staticmethod
    def object_key(_context: ErasedCallableContext, var _arguments: Tuple[Int]) -> JsString:
        return JsString()

    @staticmethod
    def object_value(_context: ErasedCallableContext, var _arguments: Tuple[Int]) -> JsValue:
        return JsValue()

    @staticmethod
    def shrink(context: ErasedCallableContext, var _arguments: Tuple[String]) raises -> JsValue:
...
def main() raises:
    ...
    var environment = allocate_callable_environment(ArrayView(owner), ArrayView.destroy)
    ...
    var item = js_value_from_source_object(
        ...
        RaisingCallable[Tuple[String], JsValue](item_environment, ItemView.shrink),
    )
    owner[].values.append(item)
    owner[].values.append(JsValue(1.0))
    assert_equal(json_stringify_with_space_number(array, 2).value().to_native_strict(), '[\n  9,\n  null\n]')
