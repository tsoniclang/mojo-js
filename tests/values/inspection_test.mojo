...
def main() raises:
    ...
    var object = builder.append_object([JsString("items"), JsString("not plain")], [array, name])
    assert_equal(inspect_value(builder.value(array)), "[ 1, 'Alice' ]")
    assert_equal(inspect_value(builder.value(object)), "{ items: [ 1, 'Alice' ], 'not plain': 'Alice' }")
    assert_equal(inspect_value(builder.value(object), 0), "{ items: [Array], 'not plain': 'Alice' }")
    assert_equal(inspect_value(builder.value(array), 2, 1), "[ 1, ... 1 more item ]")
