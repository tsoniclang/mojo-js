...
def options(owner: ArcPointer[OptionOwner]) -> JsValue:
    var context = allocate_callable_environment(
        OptionView(owner), OptionView.destroy
    )
...
def main() raises:
    var number = ArcPointer(
        OptionOwner(
            json_parse(
                JsString(
                    '{"style":"currency","currency":"usd","numberingSystem":"latn"}'
                )
            ),
            List[String](),
        )
    ...
    assert_true(
        first_read(number, "trailingZeroDisplay")
        < first_read(number, "compactDisplay")
    )
    assert_true(
        first_read(number, "compactDisplay") < first_read(number, "useGrouping")
    )
    var date = ArcPointer(
        OptionOwner(
            json_parse(
                JsString(
                    '{"calendar":"gregory","numberingSystem":"latn","timeZone":"UTC","year":"numeric"}'
                )
            ),
            List[String](),
        )
    ...
    assert_reads(
        date,
        "localeMatcher|calendar|numberingSystem|hour12|hourCycle|timeZone|weekday|era|year|month|day|dayPeriod|hour|minute|second|fractionalSecondDigits|timeZoneName|formatMatcher|dateStyle|timeStyle",
    )
    var collator = ArcPointer(
        OptionOwner(
            json_parse(
                JsString(
                    '{"collation":"phonebk","numeric":true,"caseFirst":"upper","sensitivity":"base"}'
                )
            ),
            List[String](),
        )
    ...
    assert_reads(
        collator,
        "usage|localeMatcher|collation|numeric|caseFirst|sensitivity|ignorePunctuation",
    )
    number[].reads.clear()
    var number_formatter = IntlNumberFormat(
        JsValue(JsString("en")), options(number)
    )
    ...
    date[].reads.clear()
    var date_formatter = IntlDateTimeFormat(
        JsValue(JsString("en")), options(date)
    )
    ...
    var retained_collator = IntlCollator(
        JsValue(JsString("en")), options(collator)
    )
