...
from tsonic_runtime import Callable, ErasedCallableContext, Location, RaisingCallable, WeakReferenceIdentity, allocate_callable_environment, destroy_callable_environment
from tsonic_js import JsString, JsValue, json_parse, json_stringify, json_stringify_with_property_list, json_stringify_with_property_list_and_space_number, json_stringify_with_property_list_and_space_string, js_value_from_source_object, object_keys
...
struct GetterView:
    ...
    def property(context: ErasedCallableContext, var arguments: Tuple[JsString]) raises -> JsValue:
        ...
        environment[].owner[].reads.write(environment[].owner[].reads.read() + 1)
        return JsValue(2.0)
...
def getter_value(reads: Location[Int]) -> JsValue:
    ...
    return js_value_from_source_object(
        ...
        property_reader=RaisingCallable[Tuple[JsString], JsValue](environment, GetterView.property),
    )
...
def main() raises:
    ...
    assert_equal(json_stringify_with_property_list(object, properties).value().to_native_strict(), '{"label":"x","id":1}')
    assert_equal(json_stringify_with_property_list_and_space_number(object, properties, 2).value().to_native_strict(), '{\n  "label": "x",\n  "id": 1\n}')
    assert_equal(json_stringify_with_property_list_and_space_string(object, properties, JsString(" ")).value().to_native_strict(), '{\n "label": "x",\n "id": 1\n}')

    var numeric = json_parse(JsString('{"0":"zero","1":"one","2":"two"}'))
    var numeric_keys = json_parse(JsString('[2,"1",2,1,false,null,{},-0]'))
    assert_equal(json_stringify_with_property_list(numeric, numeric_keys).value().to_native_strict(), '{"2":"two","1":"one","0":"zero"}')
    var nested = json_parse(JsString('{"own":{"keep":2,"drop":3},"keep":4,"drop":5}'))
    var nested_keys = json_parse(JsString('["own","keep"]'))
    assert_equal(json_stringify_with_property_list(nested, nested_keys).value().to_native_strict(), '{"own":{"keep":2},"keep":4}')
    var array = json_parse(JsString('[{"keep":1,"drop":2},3]'))
    assert_equal(json_stringify_with_property_list(array, json_parse(JsString('["keep"]'))).value().to_native_strict(), '[{"keep":1},3]')
    assert_equal(json_stringify_with_property_list(array, json_parse(JsString('[]'))).value().to_native_strict(), '[{},3]')
    ...
    assert_equal(json_stringify_with_property_list(getter, json_parse(JsString('["inherited","own","inherited"]'))).value().to_native_strict(), '{"inherited":2,"own":1}')
