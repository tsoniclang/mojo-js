...
from tsonic_runtime import ErasedCallableContext, RaisingCallable, allocate_callable_environment, destroy_callable_environment
from tsonic_js import JsString, JsValue, js_value_from_bigint, js_value_from_array_values, js_value_structured_clone, js_value_to_string, js_truthy, object_is, json_stringify, json_stringify_with_replacer_and_space_number
...
struct DecimalReplacer:
    ...
    def replace(_context: ErasedCallableContext, var arguments: Tuple[String, JsValue]) raises -> JsValue:
        return JsValue(arguments[1].bigint_value()) if arguments[1].is_bigint() else arguments[1]
...
def exact_integer[T: Writable](value: T, expected: String) raises:
    ...
    assert_true(object_is(retained, decode_structured_clone(encode_structured_clone(retained))))
...
def main() raises:
    ...
    exact_integer(Int128(-170141183460469231731687303715884105728), "-170141183460469231731687303715884105728")
    exact_integer(UInt128(340282366920938463463374607431768211455), "340282366920938463463374607431768211455")
    ...
    assert_true(object_is(js_value_from_bigint(Int64(1)), js_value_from_bigint(UInt64(1))))
