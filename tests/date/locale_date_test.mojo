...
from tsonic_js import (
    JsString, JsValue, json_parse, date_new, date_to_locale_string,
    date_to_locale_date_string, date_to_locale_time_string,
)
...
def main() raises:
    ...
    assert_equal(date_to_locale_date_string(epoch, locale, value('{"timeZone":"UTC","formatMatcher":"basic"}')), "1/1/1970")
    assert_equal(date_to_locale_time_string(epoch, locale, value('{"timeZone":"UTC","hour12":false}')), "00:00:00")
    assert_equal(date_to_locale_date_string(epoch, value('"de-DE"'), utc), "1.1.1970")
    assert_equal(date_to_locale_date_string(epoch, locale, value('{"timeZone":"-01:00"}')), "12/31/1969")
    assert_equal(date_to_locale_date_string(epoch, locale, value('{"timeZone":"utc","year":"numeric","month":"long","day":"numeric"}')), "January 1, 1970")
    assert_equal(date_to_locale_date_string(epoch, locale, value('{"timeZone":"UTC","dateStyle":"long"}')), "January 1, 1970")
    assert_true(len(date_to_locale_string(epoch)) > 0)
    assert_true(len(date_to_locale_time_string(epoch, value('"ja-JP"'), value('{"timeZone":"UTC","hour12":true}'))) > 0)
...
{"timeStyle":"short"}''')
    for selected in invalid_options.splitlines():
        var rejected = False
        try:
            _ = date_to_locale_date_string(epoch, locale, value(String(selected)))
        except:
            rejected = True
