...
def main() raises:
    ...
    assert_true(
        math.isnan(omitted.set_utc_seconds(2, Float64(FloatLiteral.nan)))
    )
    ...
    _ = omitted.set_time(1234)
    assert_true(math.isnan(omitted.set_utc_hours(1, Float64(2), None)))
    _ = omitted.set_time(1234)
    ...
    assert_equal(
        1.0 / date_new(-0.75).get_time(), Float64(FloatLiteral.infinity)
    )
    ...
    assert_equal(
        date_new(8640000000000000.0).to_iso_string().to_native_strict(),
    ...
    assert_true(math.isnan(date_utc(1970, Float64(0), None)))
    assert_true(math.isnan(date_utc(1e300)))
    assert_true(math.isnan(date_utc(2000, Float64(FloatLiteral.infinity))))
    assert_equal(date_utc(2000, Float64(12)), date_utc(2001, Float64(0)))
    assert_equal(date_utc(2000, Float64(-1)), date_utc(1999, Float64(11)))
    assert_equal(date_utc(2000, Float64(1), Float64(29)), 951782400000.0)
    ...
    var valid: List[String] = [
        ...
        "+001970-01-01T00:00:00.000Z",
        "1970-01-01T05:30:00+05:30",
        "1969-12-31T19:00:00-05:00",
        "1969-12-31T24:00:00Z",
    ...
    var rejected: List[String] = [
        ...
        "1970-01-00",
        "1970-01-32",
        "1970-01-01Txx:00:00Z",
        "1970-01-01T00:xx:00Z",
        "1970-01-01T00:00:xxZ",
        "1970-01-01T00:00:00.Z",
        "1970-01-01T00:00:00+05:60",
        "1970-01-01T00:00:00+24:00",
        "1970-01-01T24:00:01Z",
    ...
    for timestamp in examples:
        ...
        assert_equal(
            date_parse_native(date.to_iso_string().to_native_strict()),
            timestamp,
        )
        assert_equal(
            date_parse_native(date.to_utc_string().to_native_strict()),
            timestamp,
        )
        assert_equal(
            date_parse_native(date.to_string().to_native_strict()), timestamp
        )
