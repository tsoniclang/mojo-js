...
from tsonic_js import JsString, JsValue, json_parse, string_to_locale_lower_case, string_to_locale_upper_case, string_locale_compare, js_string_to_locale_lower_case
...
def main() raises:
    ...
    assert_true(string_locale_compare("file2", "file10", data('"en"'), data('{"numeric":true}')) < 0)
    assert_true(string_locale_compare("file2", "file10", data('"en-u-kn"'), data('{"numeric":false}')) > 0)
    assert_equal(string_locale_compare("ä", "a", data('"de"'), data('{"sensitivity":"base"}')), 0.0)
    assert_true(string_locale_compare("ä", "a", data('"sv"'), data('{"sensitivity":"base"}')) > 0)
    assert_equal(string_locale_compare("a-b", "ab", data('"en"'), data('{"ignorePunctuation":true}')), 0.0)
    ...
    for invalid in ['{"numeric":true,"caseFirst":""}', '{"sensitivity":"invalid"}', '{"usage":""}', '{"localeMatcher":"other"}', '{"collation":""}', 'null']:
        rejected = False
        try:
