...
def _present(
    value: JsDate, locales: JsValue, options: JsValue, required: String
) raises -> String:
...
def date_to_locale_string(
    value: JsDate, locales: JsValue = JsValue(), options: JsValue = JsValue()
) raises -> String:
    return _present(value, locales, options, "all")


def date_to_locale_date_string(
    value: JsDate, locales: JsValue = JsValue(), options: JsValue = JsValue()
) raises -> String:
    return _present(value, locales, options, "date")


def date_to_locale_time_string(
    value: JsDate, locales: JsValue = JsValue(), options: JsValue = JsValue()
) raises -> String:
