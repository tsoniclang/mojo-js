...
def _selection(
    options: JsValue, name: String, choices: String, patterns: String
) raises -> String:
...
struct DateOptions(Movable):
    ...
    def __init__(
        out self, options: JsValue, required: String, defaults: String
    ) raises:
        if (
            required != "all" and required != "date" and required != "time"
        ) or (defaults != "all" and defaults != "date" and defaults != "time"):
        ...
        self.hour_cycle = _selection(
            options, "hourCycle", "h11|h12|h23|h24", "h11|h12|h23|h24"
        )
        var zone = option_value(options, "timeZone")
        self.has_zone = not zone.is_undefined()
        self.zone = option_string(zone, "")
        if self.has_zone and (
            self.zone == ""
            or len(self.zone.as_bytes()) > 511
            or "\0" in self.zone
        ):
            raise Error("Invalid time-zone identifier")
        var weekday = _selection(
            options, "weekday", "long|short|narrow", "EEEE|EEE|EEEEE"
        )
        var era = _selection(
            options, "era", "long|short|narrow", "GGGG|G|GGGGG"
        )
        var year = _selection(options, "year", "numeric|2-digit", "y|yy")
        var month = _selection(
            options,
            "month",
        ...
        var period = _selection(
            options, "dayPeriod", "long|short|narrow", "BBBB|B|BBBBB"
        )
        ...
        if not digits.is_undefined():
            if not digits.is_number() or not (
                digits.number_value() >= 1 and digits.number_value() <= 3
            ):
                raise Error(
                    "fractionalSecondDigits must be between one and three"
                )
            for _ in range(Int(math.floor(digits.number_value()))):
                fraction += "S"
        var zone_name = _selection(
            options,
            "timeZoneName",
            "long|short|shortOffset|longOffset|shortGeneric|longGeneric",
            "zzzz|z|O|OOOO|v|vvvv",
        )
        ...
        if self.date_style != -1 or self.time_style != -1:
            if self.skeleton != "":
                raise Error(
                    "Date styles cannot be combined with component options"
                )
            ...
            if required != "time" and (
                weekday != "" or year != "" or month != "" or day != ""
            ):
                need_defaults = False
