...
def number_option(value: JsValue, minimum: Int, maximum: Int, default: Int) raises -> Int:
    if value.is_undefined():
        return default
    if not value.is_number():
        raise Error("Number formatting requires a numeric digit option")
    var number = value.number_value()
    if not math.isfinite(number) or number < Float64(minimum) or number > Float64(maximum):
        raise Error("Number formatting digit option is out of range")
    return Int(math.floor(number))


def number_choice(options: JsValue, name: String, default: String, choices: String) raises -> String:
...
def _increment(value: Int, fraction: Int) -> String:
    ...
    if len(digits) <= fraction:
        return "precision-increment/0." + "0" * (fraction - len(digits)) + digits
...
def number_precision(options: JsValue, compact: Bool, default_minimum: Int, default_maximum: Int) raises -> String:
    ...
    var increment = number_option(option_value(options, "roundingIncrement"), 1, 5000, 1)
    var increment_valid = False
    for allowed in String("1|2|5|10|20|25|50|100|200|250|500|1000|2000|2500|5000").split("|"):
    ...
    var mode = number_choice(options, "roundingMode", "halfExpand", "ceil|floor|expand|trunc|halfCeil|halfFloor|halfExpand|halfTrunc|halfEven")
    var priority = number_choice(options, "roundingPriority", "auto", "auto|morePrecision|lessPrecision")
    var trailing = number_choice(options, "trailingZeroDisplay", "auto", "auto|stripIfInteger")
    var rounding = String()
    var modes = String("ceil|floor|expand|trunc|halfCeil|halfFloor|halfExpand|halfTrunc|halfEven").split("|")
    var native_modes = String("ceiling|floor|up|down|half-ceiling|half-floor|half-up|half-down|half-even").split("|")
    for index in range(len(modes)):
        if mode == modes[index]:
            rounding = "rounding-mode-" + String(native_modes[index])
    var has_significant = not minimum_significant.is_undefined() or not maximum_significant.is_undefined()
    var has_fraction = not minimum_fraction.is_undefined() or not maximum_fraction.is_undefined()
    var need_significant = priority != "auto" or has_significant
    var need_fraction = priority != "auto" or (not has_significant and (has_fraction or not compact))
    ...
    if need_fraction:
        ...
        if minimum == -1:
            minimum = min(default_minimum, maximum) if maximum != -1 else default_minimum
        if maximum == -1:
            maximum = max(default_max, minimum)
        if minimum > maximum:
            raise Error("Minimum fraction digits exceed maximum fraction digits")
    ...
    if increment != 1:
        if need_significant or priority != "auto" or fraction_minimum != fraction_maximum:
            raise Error("Rounding increments require equal fraction digits and automatic fraction precision")
