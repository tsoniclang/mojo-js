...
struct IntlResult(Movable):
    ...
    def part_count(self) raises -> Int:
        self.check()
        var count = Int(external_call["tsonic_js_intl_part_count", c_size_t](self.pointer.value()))
        if count < 0 or count > 4096:
            raise Error("Internationalization parts exceed their finite contract")
        return count
    ...
    def part_type(self, index: Int) raises -> String:
        ...
        var type = external_call[
            "tsonic_js_intl_part_type", OptionalPointer[UInt8, ImmUntrackedOrigin],
        ](self.pointer.value(), c_size_t(index))
        if not type:
            raise Error("Internationalization part has no native classification")
        return String(unsafe_from_utf8_ptr=type.value())

    def part_bounds(self, index: Int) raises -> Tuple[Int, Int]:
        if index < 0 or index >= self.part_count():
            raise Error("Internationalization part index is outside its result")
        var start = Int(external_call["tsonic_js_intl_part_start", c_size_t](self.pointer.value(), c_size_t(index)))
        var length = Int(external_call["tsonic_js_intl_part_length", c_size_t](self.pointer.value(), c_size_t(index)))
        var total = Int(external_call["tsonic_js_intl_length", c_size_t](self.pointer.value()))
        if start < 0 or length < 0 or start > total or length > total - start:
            raise Error("Internationalization part has invalid native positions")
        return (start, length)
