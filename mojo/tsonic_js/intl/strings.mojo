...
def js_string_to_locale_lower_case(
    value: JsString, locales: JsValue = JsValue()
) raises -> JsString:
    return JsString(
        code_units=convert_case_units(
            value._copy_code_units(), case_locale(locales), False
        )
    )


def js_string_to_locale_upper_case(
    value: JsString, locales: JsValue = JsValue()
) raises -> JsString:
    return JsString(
        code_units=convert_case_units(
            value._copy_code_units(), case_locale(locales), True
        )
    )


def string_to_locale_lower_case(
    value: String, locales: JsValue = JsValue()
) raises -> String:
    return js_string_to_locale_lower_case(
        JsString(value), locales
    ).to_native_strict()


def string_to_locale_upper_case(
    value: String, locales: JsValue = JsValue()
) raises -> String:
    return js_string_to_locale_upper_case(
        JsString(value), locales
    ).to_native_strict()
...
def string_locale_compare(
    ...
    return js_string_locale_compare(
        JsString(left), JsString(right), locales, options
    )
