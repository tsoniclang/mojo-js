...
struct JsString(Equatable, ImplicitlyCopyable, Sized):
    ...
    def ends_with(self, value: Self, end_position: Float64 = Float64(FloatLiteral.infinity)) -> Bool:
        var end = len(self) if end_position == Float64(FloatLiteral.infinity) else _clamp_index(_string_integer(end_position), len(self))
    ...
    def last_index_of(self, value: Self, position: Float64 = Float64(FloatLiteral.infinity)) -> Float64:
        var start = len(self) if position == Float64(FloatLiteral.infinity) else _clamp_index(_string_integer(position), len(self))
        if len(value) == 0:
            return Float64(start)
    ...
    def slice(self, start: Float64 = 0, end: Float64 = Float64(FloatLiteral.infinity)) -> Self:
        ...
        var last = len(self) if end == Float64(FloatLiteral.infinity) else _relative_string_index(end, len(self))
        if last < first:
            last = first
        return self._range(first, last)

    def substring(self, start: Float64, end: Float64 = Float64(FloatLiteral.infinity)) -> Self:
        var first = _clamp_index(_string_integer(start), len(self))
        var last = len(self) if end == Float64(FloatLiteral.infinity) else _clamp_index(_string_integer(end), len(self))
        if first > last:
            var swap = first
            first = last
            last = swap
    ...
    def substr(self, start: Float64, length: Float64 = Float64(FloatLiteral.infinity)) -> Self:
        var first = _relative_string_index(start, len(self))
        var count = len(self) - first if length == Float64(FloatLiteral.infinity) else max(_string_integer(length), 0)
    ...
    def trim_start(self) -> Self:
        var first = 0
        while first < len(self) and _is_js_whitespace(self._code_units[][first]):
            first += 1
    ...
    def _matches_at(self, value: Self, start: Int) -> Bool:
        ...
        for offset in range(len(value)):
            if self._code_units[][start + offset] != value._code_units[][offset]:
                return False
        return True
...
fn _string_integer(value: Float64) -> Int:
    if value != value or value == 0:
        return 0
    return Int(value)


fn _clamp_index(value: Int, length: Int) -> Int:
    return min(max(value, 0), length)


fn _relative_string_index(value: Float64, length: Int) -> Int:
