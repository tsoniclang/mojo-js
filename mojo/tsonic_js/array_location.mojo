...
](owner: JsArray[T], index: Int) raises -> T:
    return owner.read_value(index)


def _write_element[
    T: Copyable & Deinitable
](mut owner: JsArray[T], index: Int, var value: T) raises:
    owner.set(index, value^)


def array_location[
    T: Copyable & Deinitable
](owner: JsArray[T], index: Float64) raises -> TypedLocation[T]:
    ...
    return access_location[JsArray[T], Int, T](
        owner, selected_index, identity, _read_element[T], _write_element[T]
    )
