...
static const char *part_type(int32_t field, int negative, int special, int unit_style) {
    switch (field) {
        ...
        case UNUM_PERCENT_FIELD: return unit_style ? "unit" : "percentSign";
...
int tsonic_intl_number_parts(TsonicIntlResult *result,
    ...
    while ((field = ufieldpositer_next(positions, &start, &end)) >= 0) {
        const char *type = part_type(field, negative, special, unit_style);
